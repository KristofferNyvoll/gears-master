var world_FireRescue = new (function () {
  ...
  this.setOptions = function (options) {
    ...
    if (
      typeof self.options.startPosXY != "undefined" &&
      self.options.startPosXY.trim() != ""
    ) {
    ...
    if (
      typeof self.options.startRot != "undefined" &&
      self.options.startRot.trim() != ""
    ) {
      self.robotStart.rotation.y =
        (parseFloat(self.options.startRot) / 180) * Math.PI;
    } else {
  ...
  this.loadWarehouse = function (scene) {
    ...
    let cratesMeshes = self.addCubeCrates(
      scene,
      "textures/maps/Fire Rescue/woodenCrate.png",
      crates
    );

    // Auto-door
    let sensorMat = babylon.getMaterial(scene, "2626CC4D");
    let doorSensor = self.addBox(
      scene,
      sensorMat,
      [25, 25, 20],
      [-75.1, 124.5],
    ...
    let doorSensorIndicator = self.addBox(
      scene,
      sensorMat,
      [25, 25, 2],
      [-75.1, 124.5],
    ...
    var animateSensor = new BABYLON.Animation(
      "doorSensor",
      "position.y",
      10,
    ...
    breakableWalls.forEach(function (breakableWall) {
      self.addBox(scene, wallMat, breakableWall[0], breakableWall[1], false, {
        mass: 200,
        friction: 0.5,
      });
    });
    ...
    self.game.startMesh = self.addBox(
      scene,
      null,
      [27, 27, 30],
      [179.5, -132],
    ...
    self.game.redRescueMesh = self.addBox(
      scene,
      null,
      [35, 30, 0.4],
      [49.5, -132],
    ...
    self.game.greenRescueMesh = self.addBox(
      scene,
      null,
      [35, 30, 0.4],
      [136.5, -132],
  ...
  this.renderDefault = function (delta) {
    ...
    if (self.game.state == "ready") {
      if (
        self.game.startMesh.intersectsPoint(robot.body.absolutePosition) ==
        false
      ) {
      ...
      self.game.victims.forEach(function (victim, i, victims) {
        if (victim == null) {
          ...
          self.game.redRescueMesh.intersectsPoint(victim.absolutePosition)
        ) {
          if (victim.color == "red") {
            self.game.red += 1;
            self.game.score += 10;
          ...
          self.game.greenRescueMesh.intersectsPoint(victim.absolutePosition)
        ) {
          ...
          self.game.redAmbulanceMesh &&
          self.game.redAmbulanceMesh.intersectsPoint(victim.absolutePosition)
        ) {
          if (victim.color == "red") {
            self.game.red += 1;
            self.game.score += 15;
          ...
          self.game.greenAmbulanceMesh &&
          self.game.greenAmbulanceMesh.intersectsPoint(victim.absolutePosition)
        ) {
          if (victim.color == "green") {
            self.game.green += 1;
            self.game.score += 10;
  ...
  this.drawWorldInfoDefault = function (rebuild) {
    if (rebuild) {
      ...
      self.infoPanel.$red.on("animationend", function () {
        this.classList.remove("animate");
      });
      self.infoPanel.$green.on("animationend", function () {
        this.classList.remove("animate");
      });
      self.infoPanel.$score.on("animationend", function () {
        this.classList.remove("animate");
      });
    }
  ...
  this.addSpriteFire = function (scene, positions, size) {
    var spriteManagerFire = new BABYLON.SpriteManager(
      "FireManager",
      "textures/maps/Fire Rescue/fire.png",
      10,
      { width: 15, height: 20 },
      scene
    );
    ...
    positions.forEach(function (position) {
      ...
      fire.mesh = BABYLON.MeshBuilder.CreateCylinder(
        "fire",
        { height: 20, diameter: size / 3 },
        scene
      );
  ...
  this.addRampZ = function (
    ...
    let size = [width, boxLength, thickness];
    ...
    let mesh = self.addBox(scene, mat, size, pos, false, true, true, [
      rot,
      0,
      0,
    ]);
  ...
  this.addRampX = function (
    ...
    let rot = Math.atan(rampHeight / rampBase);
    let boxLength = Math.sqrt(rampBase ** 2 + rampHeight ** 2);
    let size = [boxLength, width, thickness];
    pos[2] = pos[2] - thickness / 2;
    pos[2] = pos[2] + rampHeight / 2;
    pos[2] = pos[2] - (Math.cos(rot) * thickness) / 2;
